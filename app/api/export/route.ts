...
import { STEPS, getStepByKey, CASE_META } from "@/data/case.js";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const filename = `Mayumi_PBL_${(session.displayName || "student").replace(/\s+/g, "_")}_${now.toISOString().slice(0, 10)}.docx`;
